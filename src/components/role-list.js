  ...
  return (
    <div className={styles.root}>
      ...
      <ul className={styles.list}>
        {items.map((item) => (
          <Link key={item._key} to={`/schools/${item.person.school.slug.current}/students/${item.person.slug.current}`}>
            <li className={styles.listItem}>
              <div>
                <div className={styles.avatar}>
                  {item.person && item.person.image && item.person.image.asset && (
                    <img
                      src={imageUrlFor(buildImageObj(item.person.image)).width(100).height(100).fit('crop').url()}
                      alt=""
                    />
                  )}
                </div>
